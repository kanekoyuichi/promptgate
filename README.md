...
**デフォルト設定**: `PromptGate()` はルールベース検出（正規表現・フレーズマッチ）のみ有効です。明示的なフレーズを使った直接攻撃のスクリーニングに適しています。意味的な言い換えや文脈依存の攻撃には `embedding` または `llm_judge` の追加が必要です（[スキャナーの種類](#スキャナーの種類) 参照）。
