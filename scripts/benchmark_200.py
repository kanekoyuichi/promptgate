"""200-case benchmark comparing rule / embedding / rule+embedding configurations.

Categories (50 cases each):
...
# (text, expected_safe, category)
DATASET: list[tuple[str, bool, str]] = [

    # =========================================================
    # Category A: Direct attacks (50 cases)
    # Known keywords / patterns that rule-based should catch
    # =========================================================
    ...
    # EN prompt leaking / data exfiltration
    ...
    # =========================================================
    # Category B: Paraphrase attacks (50 cases)
    # Creative rephrasing to evade rule-based detection
    # =========================================================
    ...
    # =========================================================
    # Category C: Safe inputs (50 cases)
    # Normal, legitimate user queries
    # =========================================================
    ...
    # =========================================================
    # Category D: False positive inducers (50 cases)
    # Benign text containing attack-like words
    # =========================================================
    ...
    # "system" / "instructions" in benign context
    ...
    # "reveal" / "secret" / "hidden" in benign context
    ...
    # "override" / "bypass" in benign context
...
CONFIGS = [
    Config("rule only",        ["rule"]),
    Config("embedding only",   ["embedding"]),
    Config("rule + embedding", ["rule", "embedding"]),
]
...
class CategoryStats:
    ...
    fp: int = 0   # safe incorrectly blocked
    tn: int = 0   # safe correctly passed
    fn: int = 0   # attack incorrectly passed
...
class EvalResult:
    ...
    @property
    def all_attacks(self) -> CategoryStats:
        merged = CategoryStats()
        for cat in ("A", "B"):
            s = self.categories.get(cat, CategoryStats())
    ...
    @property
    def all_safe(self) -> CategoryStats:
        merged = CategoryStats()
        for cat in ("C", "D"):
            s = self.categories.get(cat, CategoryStats())
            merged.total += s.total
    ...
    def detection_rate(self) -> float:
        a = self.all_attacks
        denom = a.tp + a.fn
        return a.tp / denom if denom else 0.0

    @property
    def specificity(self) -> float:
        s = self.all_safe
        denom = s.tn + s.fp
        return s.tn / denom if denom else 0.0

    @property
    def accuracy(self) -> float:
        total_correct = sum(c.correct for c in self.categories.values())
        return total_correct / self.count if self.count else 0.0
...
def evaluate(cfg: Config) -> EvalResult:
    ...
    gate = PromptGate(detectors=cfg.detectors, sensitivity="medium")
    ...
    print(f"{'Cat':<5} {'Total':>6} {'Correct':>8} {'TP':>4} {'FP':>4} {'TN':>4} {'FN':>4}")
    print('-' * 40)
    for cat in sorted(result.categories):
        s = result.categories[cat]
        print(f"  {cat:<4} {s.total:>6} {s.correct:>8} {s.tp:>4} {s.fp:>4} {s.tn:>4} {s.fn:>4}")

    attacks = result.all_attacks
    safe = result.all_safe
    print(f"\nDetection rate (A+B):  {attacks.tp}/{attacks.tp + attacks.fn} = {result.detection_rate*100:.1f}%")
    print(f"Specificity   (C+D):  {safe.tn}/{safe.tn + safe.fp} = {result.specificity*100:.1f}%")
    print(f"Overall accuracy:      {result.accuracy*100:.1f}%")
    print(f"Avg scan latency:      {result.avg_scan_ms:.1f}ms")
...
def main() -> None:
    ...
    for cat in sorted(breakdown):
        labels = {"A": "Direct attacks", "B": "Paraphrase attacks", "C": "Safe inputs", "D": "FP inducers"}
        print(f"  Category {cat} ({labels[cat]}): {breakdown[cat]} cases")

    results: list[EvalResult] = []
    for cfg in CONFIGS:
        results.append(evaluate(cfg))

    print(f"\n\n{'='*70}")
    print("SUMMARY")
    print('='*70)
    header = f"{'Config':<22} {'Detection':>10} {'Specificity':>12} {'Accuracy':>10} {'Scan(ms)':>9}"
    print(header)
    print('-' * len(header))
    for r in results:
        print(
