"""Fair comparison of three embedding models using the 200-case dataset.

Test methodology (ref: https://qiita.com/kanekoyuichi/items/46ea62f85f7784601296):
  200 test cases split into 4 categories (50 each):
    A - Direct attacks   : known patterns, rule-friendly keywords
    B - Paraphrase attacks: creative rephrasing to evade rules
    C - Safe inputs      : legitimate user queries
    D - FP inducers      : benign text containing attack-like words

  Metrics:
    Detection rate  = TP / (TP + FN)  on categories A+B
    Specificity     = TN / (TN + FP)  on categories C+D
    Overall accuracy across all 200 cases

Configurations tested (same dataset, same sensitivity="medium"):
  - embedding only  (rule disabled)
  - rule + embedding
"""
...
SHORT_NAMES = {
    "paraphrase-multilingual-MiniLM-L12-v2": "MiniLM-L12-v2",
    "paraphrase-multilingual-mpnet-base-v2": "mpnet-base-v2",
    "intfloat/multilingual-e5-large":        "e5-large",
}

# ---------------------------------------------------------------------------
# Dataset (200 cases, 50 per category)
# ---------------------------------------------------------------------------

DATASET: list[tuple[str, bool, str]] = [

    # =====================================================================
    # A: Direct attacks (50 cases) — known keywords / rule-friendly
    # =====================================================================
    ...
    # =====================================================================
    # B: Paraphrase attacks (50 cases) — creative rephrasing to evade rules
    # =====================================================================
    ...
    # =====================================================================
    # C: Safe inputs (50 cases) — legitimate user queries
    # =====================================================================
    ...
    # =====================================================================
    # D: FP inducers (50 cases) — benign text with attack-like keywords
    # =====================================================================
...
assert len(DATASET) == 200, f"Expected 200 cases, got {len(DATASET)}"
...
class EvalResult:
    ...
    def add(self, cat: str, expected_safe: bool, got_safe: bool, ms: float) -> None:
        s = self.categories.setdefault(cat, CategoryStats())
        s.total += 1
        self.count += 1
    ...
    def specificity(self) -> float:
        ...
        fp_count = sum(self.categories.get(c, CategoryStats()).fp for c in ("C", "D"))
        return tn / (tn + fp_count) if (tn + fp_count) else 0.0
...
def build_configs() -> list[Config]:
    ...
    for model in MODELS:
        ...
        configs.append(Config(f"{short} | emb only",   model, ["embedding"]))
        configs.append(Config(f"{short} | rule+emb",   model, ["rule", "embedding"]))
    return configs
...
def evaluate(cfg: Config) -> EvalResult:
    ...
    result = EvalResult(label=cfg.label, load_time_s=load_time)
    ...
    for cat in sorted(result.categories):
        ...
        label = {"A": "Direct atk", "B": "Paraphrase ", "C": "Safe       ", "D": "FP inducers"}[cat]
        print(f"  {cat}({label}) {s.total:>6} {s.tp:>4} {s.fp:>4} {s.tn:>4} {s.fn:>4}")

    print(f"\n  Detection  (A+B): {result.detection_rate*100:5.1f}%")
    print(f"  Specificity(C+D): {result.specificity*100:5.1f}%")
    print(f"  Accuracy        : {result.accuracy*100:5.1f}%")
    print(f"  Avg scan        : {result.avg_scan_ms:.1f}ms")
    return result
...
def main() -> None:
    ...
    for cat in sorted(breakdown):
        names = {"A": "Direct attacks", "B": "Paraphrase attacks", "C": "Safe inputs", "D": "FP inducers"}
        print(f"  Category {cat} ({names[cat]}): {breakdown[cat]}")
    ...
    w = 30
    print(f"\n\n{'='*80}")
    print("SUMMARY  (sensitivity=medium, dataset=200)")
    print('='*80)
    header = (
        f"{'Config':<{w}} {'Detection':>10} {'Specificity':>12} "
        f"{'Accuracy':>10} {'Scan(ms)':>9}"
    )
    print(header)
    print('-' * len(header))
    ...
    for r in results:
        print(
            f"{r.label:<{w}} "
            f"{r.detection_rate*100:>9.1f}% "
            f"{r.specificity*100:>11.1f}% "
            f"{r.accuracy*100:>9.1f}% "
            f"{r.avg_scan_ms:>9.1f}"
        )

    # Model-level summary (rule+emb only)
    print(f"\n--- rule+embedding comparison ---")
    print(f"{'Model':<22} {'Detection':>10} {'Specificity':>12} {'Accuracy':>10} {'Scan(ms)':>9}")
    print('-' * 65)
    for r in results:
        if "rule+emb" in r.label:
            model_short = r.label.split(" |")[0]
            print(
                f"{model_short:<22} "
                f"{r.detection_rate*100:>9.1f}% "
                f"{r.specificity*100:>11.1f}% "
